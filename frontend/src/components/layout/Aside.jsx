...
function PreviewModal({ project, onClose }) {
  ...
  return createPortal(
    <div
      ...
      className="modal-overlay fixed inset-0 z-[9999] bg-black/70 backdrop-blur-sm flex items-center justify-center p-4"
    >
      <div className="modal-panel bg-gray-900 border border-gray-700 rounded-2xl w-full max-w-2xl max-h-[80vh] flex flex-col shadow-2xl">
  ...
  return (
    <div className="rounded-lg overflow-hidden group">
      ...
      <button
        ...
        className={`flex items-center gap-2 rounded-lg hover:bg-slate-800 transition-all duration-300 ease-in-out text-left ${
          sidebarOpen ? "w-full h-11 px-3 mx-1" : "h-10 w-10 mx-auto justify-center"
        }`}
      >
        {/* Icono del proyecto */}
...
export default function Aside({
  ...
  return (
    <>
      ...
      <aside
        className={`sidebar fixed left-0 top-0 h-screen bg-slate-950 border-r border-slate-800 flex flex-col overflow-hidden z-10 ${isOpen ? "px-3" : ""} ${
          isOpen ? "w-64 shadow-xl shadow-black/20" : "w-16"
        }`}
      >
        {/* ── HEADER LOGO ── */}
        <div className="flex items-center h-16 flex-shrink-0">
          <button
            onClick={toggleAside}
            className={`flex items-center gap-2 h-10 rounded-lg hover:bg-slate-800 transition-all duration-300 ease-in-out text-slate-300 hover:text-white ${
              isOpen ? "w-full px-2 justify-start" : "w-10 mx-auto justify-center"
            }`}
          >
        ...
        <div className={`pb-3 flex ${isOpen ? "" : "justify-center"}`}>
          <button
            onClick={onNewProject}
            className={`flex items-center justify-center gap-2 h-11 rounded-lg bg-gradient-to-r from-indigo-600 to-indigo-500 text-white font-medium text-sm hover:from-indigo-500 hover:to-indigo-400 transition-all duration-300 ease-in-out shadow-md hover:shadow-lg hover:scale-[1.02] active:scale-[0.98] ${
              isOpen ? "w-full" : "w-10"
            }`}
          >
            <svg className="w-5 h-5 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            {isOpen && <span className="sidebar-text">Nuevo proyecto</span>}
          </button>
        </div>
        ...
        <div className="py-3 flex flex-col gap-1">
          ...
          {/* Avatar + info */}
          <div className={`flex items-center h-11 rounded-lg ${isOpen ? "px-3" : "w-10 mx-auto justify-center"}`}>
          ...
          <button
            onClick={onSettings}
            className={`flex items-center gap-2 h-10 rounded-lg hover:bg-slate-800 transition-all duration-300 ease-in-out text-slate-400 hover:text-slate-200 ${
              isOpen ? "w-full px-3 justify-start" : "w-10 mx-auto justify-center"
            }`}
          >
            <svg className="w-5 h-5 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2}
            ...
            {isOpen && <span className="text-sm sidebar-text">Configuración</span>}
          </button>

          {/* Cerrar sesión */}
          <button
            onClick={onLogout}
            className={`flex items-center gap-2 h-10 rounded-lg hover:bg-red-500/10 transition-all duration-300 ease-in-out text-red-400 hover:text-red-300 ${
              isOpen ? "w-full px-3 justify-start" : "w-10 mx-auto justify-center"
            }`}
          >
            <svg className="w-5 h-5 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            {isOpen && <span className="text-sm sidebar-text">Cerrar sesión</span>}
          </button>
        </div>
      </aside>
    </>
