...
**Versión:** 1.0 · **Fecha:** 2026-05-02 22:30:50 · **Generado con MoSCoW AI** · **Requisitos:** 11
...
El presente documento de especificación de requisitos para el proyecto SkyRoute Logistics tiene como objetivo definir las funcionalidades y requisitos del sistema para garantizar su implementacion exitosa y cumplimiento de las necesidades del negocio. Está dirigido a los interesados en el proyecto, incluyendo al equipo de desarrollo y la gerencia.

### 1.2 Alcance del Sistema

El sistema SkyRoute Logistics se enfoca en la gestión de rutas de vuelo para drones, evitando zonas de exclusión y garantizando un uptime del 99.99% para evitar drones varados en el aeropuerto. Cubre las necesidades de los operadores humanos y del centro de control, permitiendo la toma de decisiones informadas y la reprogramacion automática de entregas.
...
SkyRoute Logistics es un sistema de gestón de rutas de vuelo para drones que utiliza tecnología de punta para optimizar las rutas y garantizar la entrega segura y eficiente. El sistema se integra con sensores del drone y el centro de control, permitiendo la toma de decisiones informadas y la reprogramacion automática de entregas. El valor aportado es la mejora en la eficiencia y seguridad de las operaciones logisticas.
...
|--------|-----|-------------------|
| **Ana Perez** | Product Manager | Define el backlog y prioridades |
...
| **Clasificación MoSCoW** | Could Have |
...
| **Clasificación MoSCoW** | Won't Have |
...
|----|-------------|-----------|-------|---------------|
| RNF-01 | El sistema debe mantener un uptime del 99,99% para evit... | **Must Have** | 3.4 | Must Have — score 3.4/5: alto impacto de negocio (5/5). Requisito crítico para el MVP. |
| RF-01 | El sistema debe calcular la ruta de vuelo más corta evi... | **Must Have** | 3.3 | Must Have — score 3.3/5: alto impacto de negocio (4/5). Requisito crítico para el MVP. |
| RF-02 | El sistema debe permitir al operador humano tomar el co... | **Must Have** | 3.3 | Must Have — score 3.3/5: alto impacto de negocio (4/5). Requisito crítico para el MVP. |
| RF-03 | El sistema debe reprogramar automáticamente la entrega ... | **Should Have** | 3.3 | Should Have — score 3.3/5: alto impacto de negocio (4/5). |
| RD-01 | El sistema debe evitar que los drones vuelen a una altu... | **Should Have** | 3.3 | Should Have — score 3.3/5: alto impacto de negocio (4/5), bajo coste de implementación (2/... |
| RF-04 | El sistema debe bloquear automáticamente todos los desp... | **Should Have** | 3.3 | Should Have — score 3.3/5: alto impacto de negocio (4/5). |
| RD-02 | El sistema debe rechazar cualquier ruta que comprometa ... | **Should Have** | 3.3 | Should Have — score 3.3/5: alto impacto de negocio (4/5), bajo coste de implementación (2/... |
| RNF-03 | El sistema debe verificar la firma digital de todos los... | **Could Have** | 3.1 | Could Have — score 3.1/5: alto impacto de negocio (4/5), esfuerzo de implementación elevad... |
| RNF-05 | El sistema no permitirá el despacho de pedidos que supe... | **Could Have** | 2.9 | Could Have — score 2.9/5: bajo impacto de negocio (2/5), bloquea múltiples componentes del... |
| RNF-02 | El sistema debe garantizar una latencia de comunicación... | **Could Have** | 2.8 | Could Have — score 2.8/5 |
| RNF-04 | El backend debe ser capaz de coordinar hasta 1,000 dron... | **Won't Have** | 2.8 | Won't Have — score 2.8/5 No se implementará en este ciclo de desarrollo. |
...
*Generado con MoSCoW AI · 2026-05-02 22:30:50*